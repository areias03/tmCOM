...
m_list = [ec_bt, ec_bu, ec_ec, ec_fn, ec_ri, ec_sp, ec_ss]
...
def prot_pool_analysis(model: Model):
    ...
    print(f'Finished iteration for {model_id}')
    return df
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor() as p:
        ...
        df.to_csv('../data/model_tuning/prot_pool_all2.csv')
        print(df)
        df_norm = (df - df.mean()) / df.std()
        sns.heatmap(df_norm)
        plt.savefig('../data/model_tuning/prot_pool_all_hm.png')
